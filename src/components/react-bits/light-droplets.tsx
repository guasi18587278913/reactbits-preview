...
const LightDroplets: React.FC<LightDropletsProps> = ({
  ...
  useEffect(() => {
    ...
    // Observe the container directly so the canvas resizes with its parent
    // even when the viewport doesn't change. Without this, mounting inside
    // a layout that finishes sizing after the effect runs (flex/grid
    // settling, font swap, late image load) leaves the canvas frozen at
    // the initial undersized rect, producing blank strips on the right
    // and bottom edges.
